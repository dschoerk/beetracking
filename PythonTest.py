...
inputvids = ['1.tracking/count-hard.mp4', 
    ...
    # video 4 ghosts
for selectedVideoIdx in range(3,12):
    ...
    while(True):
        ...
        bluemask = image_col[:,:,0] < 0.5
        redmask = image_hsv_f[:,:,1] > 0.6
        imgmask = ((redmask & bluemask) * 255).astype(np.uint8)
        ...
        for i in range(n_components):
            ...
            if(area > 800 and area < 3500):
                ...
                if smallest_distance > 60:
                    newtracker = BeeTracker(centroid, (track_window_w, track_window_h), image_hsv)
                    trackers.append(newtracker)
                #else:
                #    cv2.circle(image_small_show, centroid, 10, (255,0,0), -1)
          
            if(area < 800):
                labeled_image[labels == i] = 0 # remove too small connected components
                #imgmask_filled[labels == i] = 0

        cv2.imshow("finalmask", imgmask_filled)
        cv2.imshow('new tracker candidate blobs', ((labeled_image > 0)*255).astype(np.uint8))
        ...
        #cv2.imshow('frame', imgmask)
        #imgmask = cv2.morphologyEx(imgmask, cv2.MORPH_ERODE, kernel)
        #cv2.imshow('frame2', imgmask)
    
        cv2.imshow('frame3', image_small_show)
        cv2.imshow('trajectories', trajectories)
    
        #cv2.imshow('image_small', image_small)
        #cv2.imshow('frame4', labeled_image)
